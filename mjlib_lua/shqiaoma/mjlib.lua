...
M.HU_TYPE_MULTIPLE = {
    ...
    [M.HU_TYPE.HUNYISE]        =  2,
    [M.HU_TYPE.PENGHU]          = 2,
    [M.HU_TYPE.QINGYISE]        = 4,
    [M.HU_TYPE.HUNPENG]       = 4,
    [M.HU_TYPE.QINGPENG]     = 8,
    [M.HU_TYPE.QIAGNGANG]  =3
}
...
function M.check_eye(cards)
    ...
    -- print(tostring(ret)..'========'..tostring(eye))
    --     utils.print(cards)
    if not ret then
        return false
    end
    ...
    begin = 28
    ret, eye = M._check_feng_eye(cards, begin, begin+3, eye_begin ~= nil)
    if not ret then
        return false
    end
...
-- dui==true   have dui
    ...
    elseif yu == 2 then
        ...
        return true, true
    elseif yu == 0 then
--  ==0 have no this type ,check next
        return true, false
    end
